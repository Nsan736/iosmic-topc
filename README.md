...
プレビューウィンドウに iPad の映像が出る。ウィンドウを閉じても受信は続き、終了はウィンドウ上で `q` / `Esc` か、ターミナルで Ctrl+C。起動時に表示される「PC の IP」を iPad のアプリに入力する。
...
- 送信中は「詳細 (不具合調査用)」を開くと、接続の状態、最後にカメラのフレームが届いた時刻、送信が止まっていないか、メモリ使用量などが 1 秒ごとに見られる。止まったときはこの画面を見れば、カメラ・送信・接続のどこで止まったかが分かる
- 接続が 6 秒終わらない、1 フレームの送信が 5 秒終わらない、カメラのフレームが 4 秒届かない、のいずれかが起きると自動で立て直す
...
| PC のプレビューが止まり、警告も出ない | `windows\video_receiver.log` を見る。PC の表示処理が止まっていれば `[注意] PC の表示処理が…止まっています` と止まった行が記録される |
| 映像がカクつく | 解像度を 720p や 480p に下げるか、画質を下げる。アプリの fps 表示が 30 近く出ていれば送信側は間に合っている |
| Safari では PC に届くのに、アプリだけ「PC から応答がない」になる | iPad が Wi-Fi を繋ぎ直して IP が変わった後に起きやすい。アプリは Wi-Fi 以外 (モバイル通信や StikDebug の VPN) を使わない設定になっている。カメラ欄の「詳細」の「経路」で、送信元の IP が今の iPad の IP と合っているか確認する。合っていなければアプリを完全に終了して開き直す |
